...
pub struct FuncionarioFiltersState {
    pub id: Option<u32>,
...
#[component]
pub fn FuncionarioFilters(
    on_filter: Arc<dyn Fn(FuncionarioFiltersState) + Send + Sync>,
    ...
    let has_active_filters = move || {
        !id.get().trim().is_empty()
            ...
            || !matricula.get().trim().is_empty()
            || !cpf.get().trim().is_empty()
            || !grupo.get().trim().is_empty()
    ...
    let handle_filter = {
        ...
        move |_| {
            on_filter(FuncionarioFiltersState {
                id: parse_u32(&id.get()),
                nome: clean_text(nome.get()),
                matricula: clean_text(matricula.get()),
                cpf: clean_digits(cpf.get()),
                grupo: clean_text(grupo.get()),
    ...
    let loading_matricula = loading.clone();
    let loading_cpf = loading.clone();
    let loading_grupo = loading.clone();
        ...
        <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            ...
            >
                <div class="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    ...
                    <div class="space-y-2">
                        <label class=label_class>"Matricula"</label>
                        <input
                            class=input_class
                            type="text"
                            placeholder="Buscar por matricula..."
                            prop:value=move || matricula.get()
                            disabled=move || loading_matricula()
                    ...
                    <div class="space-y-2">
                        <label class=label_class>"Grupo"</label>
                        <input
                            class=input_class
                            type="text"
                            placeholder="1,2,3"
                            prop:value=move || grupo.get()
                            disabled=move || loading_grupo()
                            on:input=move |event| set_grupo.set(event_target_value(&event))
                        />
                    </div>
